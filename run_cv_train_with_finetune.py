...
Data flow:
    ...
    3. Stage 1: Main training (side-angle only, 12 epochs)
    4. Stage 2: Multi-view fine-tuning (optional, adds side+top consistency)
    
    5. Saves trained models to:
       - outputs/fold_metadata/     (fold splits and samples)
       - wandb/                     (weights and biases logs)

Usage:
    python run_cv_train_with_finetune.py     # Train all 5 folds with fine-tuning
